				...
				<div class="box12 block ">
					<div class="w-box">
						<div class="w-box-header">
							<div class="pull-left">List of Duplicated Files</div>
						</div>
								...
								<div class="span12">
									...
									<table cellpadding="10px" class="table table-striped table-bordered mediaTable ">
										...
										<tbody>
											<?php 
												$num=0;
												$batchYear = CsdbCustomerDuplicatedRaw::max('batchYear');
												$batchMonth = CsdbCustomerDuplicatedRaw::where('batchYear', $batchYear)->max('batchMonth');
												
												$monthNum = sprintf("%02s", $batchMonth);
												$timestamp = mktime(0, 0, 0, $monthNum, 10);

												$cnt = CsdbCustomerDuplicatedRaw::where('batchYear', $batchYear )->where('batchMonth', $batchMonth)->orderby('batchWeek', 'desc')->groupby('batchID')->count(); 
											?>
											@if($cnt>0)	
											@foreach(CsdbCustomerDuplicatedRaw::where('batchYear', $batchYear )->where('batchMonth', $batchMonth)->orderby('batchWeek', 'desc')->groupby('batchID')->get() as $file)

											<tr>
												<td style="text-align:center">{{++$num}}</td>
												<td>
													<a href="{{ URL::route('viewDuplicatedFile', $file->batchID) }}">
														{{$file->batchName}}
													</a>
												</td>
											...
											@else
												<tr>
													<td colspan="4"> There is no file in the system. </td>
												</tr>
											@endif
										</tbody>
									</table>
